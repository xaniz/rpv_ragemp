...
public class motoskola : Script
{
    ...
    [RemoteEvent("amskola")]
    public void amskola(Player Client, int index)
    {
        try
        {
                ...
                case 11:
                    {
                        ...
                        NAPI.Task.Run(() =>
                        {
                            if (NAPI.Player.IsPlayerConnected(Client))
                            {
                            Client.TriggerEvent("ms6");
                            Client.SetData<dynamic>("mschool_tutorial", true);
    ...
    {
        if (c.GetData<dynamic>("mschool_tutorial") == true )
        {
                ...
                if (shape.GetData<int>("LMNUMBER") != c.GetData<int>("lmpoint")) return;
                    ...
                    if (lmpoint == Checkpoints.Count - 1)
                    {
                        ...
                        if (c.IsInVehicle && veh.NumberPlate == "as"+playername)
                        {
                            ...
                            Main.SendMessageWithTagToPlayer(c, "" + Main.EMBED_WHITE + "[Auto-skola]", "Dobili ste dozvolu za motor!");
